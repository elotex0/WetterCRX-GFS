...
DATE = os.environ.get("DATE", "20251203")
RUN = os.environ.get("RUN", "00")

os.makedirs("data/tp", exist_ok=True)

FIELD_REGEX = r":\s*PRATE:surface"
...
def fetch_tp_acc(fh):
    ...
    out = f"data/tp/tp_{fh_padded}.grib2"
        ...
        try:
            ...
            for line in idx_data:
                ...
                if re.search(FIELD_REGEX, line):
                    tp_offsets.append(offset)

            if not tp_offsets:
                return f"⚠️ [{fh_padded}] Kein Niederschlag gefunden"

            head = requests.head(grib_url, timeout=10)
            filesize = int(head.headers.get("Content-Length", 0))

            ranges = []
            for start in tp_offsets:
...
forecast_hours = list(range(0, 121))
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    for fh in forecast_hours:
        tasks.append(executor.submit(fetch_tp_acc, fh))

    for future in as_completed(tasks):
        print(future.result())

print("\n🎉 COMPLETED: TP-Downloads abgeschlossen!")
